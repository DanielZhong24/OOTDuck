...
const upload = multer({ storage: multer.memoryStorage(), limits:{fileSize: 5*(10**6)} });


const checkFileSize = (err:any,req:Request,res:Response,next:NextFunction)=>{
    if(err){
...
router.post('/', upload.single('image'), checkFileSize,clothesController.addClothes);
